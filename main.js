...
add_cards.forEach(cart => {
    cart.addEventListener("click", function() {
        ...
        if (badge_value != "Cart Added") {
            ...
            const cart_price = cart.nextElementSibling.firstElementChild
            ...
            totale_cart_price.textContent = Number(totale_cart_price.textContent) + Number(cart_price);
            // Make Cart Add
...
carts.addEventListener("click", function(event) {
    if (event.target.classList.contains("remove")) {
        ...
        totale_cart_price.textContent = Number(totale_cart_price.textContent) - Number(price_ele);
...
carts.addEventListener("click", function(event) {
    if (event.target.classList.contains("angle-down")) {
        ...
        if (number_eles.textContent <= 0) {
            ...
            totale_cart_price.textContent = Number(totale_cart_price.textContent) - Number(cart_price.textContent); 
            shop_nbr.textContent = Number(shop_nbr.textContent) - 1;
        }
    }
